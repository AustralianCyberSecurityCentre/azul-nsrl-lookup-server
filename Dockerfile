...
RUN uv pip uninstall --system azul-nsrl-lookup-server
RUN uv pip install --system --no-deps --find-links /tmp/ azul-nsrl-lookup-server==$(hatchling version)
...
RUN if [ "$GIT_BRANCH_NAME" = "refs/heads/dev" ] ; then \
    uv pip freeze | grep 'azul-.*==' | grep -v '^azul-nsrl-lookup-server' | cut -d "=" -f 1 | xargs -I {} uv pip install --extra-index-url=$(UV_INDEX_URL) --system --upgrade --no-deps --prerelease allow '{}>=0.0.0-dev' \
    else \
    uv pip freeze | grep 'azul-.*==' | grep -v '^azul-nsrl-lookup-server' | cut -d "=" -f 1 | xargs -I {} uv pip install --extra-index-url=$(UV_INDEX_URL) --system --upgrade --no-deps '{}>=0.0.0'\
    ;fi
...
ARG UID=21000
ARG GID=21000
RUN groupadd -g $GID azul && useradd --create-home --shell /bin/bash -u $UID -g $GID nsrl
USER nsrl
...
USER nsrl
# test scripts will be installed to the local user bin dir. Add local bin path for the nsrl user.
ENV PATH="/home/nsrl/.local/bin:$PATH"
COPY --chown=nsrl ./tests /tmp/tests
...
COPY --chown=nsrl ./docker-scripts/download_database.bash /home/nsrl/download_database.bash
COPY --chown=nsrl ./docker-scripts/update_database.bash /home/nsrl/update_database.bash
RUN chmod +x /home/nsrl/download_database.bash /home/nsrl/update_database.bash
WORKDIR /home/nsrl
ENTRYPOINT ["azul-nsrl-lookup-server"]
