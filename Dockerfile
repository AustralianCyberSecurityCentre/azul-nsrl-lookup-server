...
RUN pip install \
    ...
    azul-nsrl-lookup-server==$(cd /tmp/src && python -m setuptools_scm)
...
RUN if [ "$GIT_BRANCH_NAME" = "refs/heads/dev" ] ; then \
    pip install --find-links /tmp/ azul-nsrl-lookup-server==$(cd /tmp/src && python -m setuptools_scm);fi
...
ARG UID=21000
ARG GID=21000
# test scripts will be installed to the local user bin dir. Add local bin path for the user.
